package com.entity.view;

import com.entity.XiaoertuinaEntity;
import com.baomidou.mybatisplus.annotations.TableName;
import org.apache.commons.beanutils.BeanUtils;
import java.lang.reflect.InvocationTargetException;
import org.springframework.format.annotation.DateTimeFormat;
import com.fasterxml.jackson.annotation.JsonFormat;
import java.io.Serializable;
import java.util.Date;

/**
 * 中医小儿推拿
 * 后端返回视图实体辅助类
 * （通常后端关联的表或者自定义的字段需要返回使用）
 */
@TableName("xiaoertuina")
public class XiaoertuinaView extends XiaoertuinaEntity implements Serializable {
    private static final long serialVersionUID = 1L;

		/**
		* 推拿类型的值
		*/
		private String xiaoertuinaValue;



	public XiaoertuinaView() {

	}

	public XiaoertuinaView(XiaoertuinaEntity xiaoertuinaEntity) {
		try {
			BeanUtils.copyProperties(this, xiaoertuinaEntity);
		} catch (IllegalAccessException | InvocationTargetException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	}



			/**
			* 获取： 推拿类型的值
			*/
			public String getXiaoertuinaValue() {
				return xiaoertuinaValue;
			}
			/**
			* 设置： 推拿类型的值
			*/
			public void setXiaoertuinaValue(String xiaoertuinaValue) {
				this.xiaoertuinaValue = xiaoertuinaValue;
			}













}
